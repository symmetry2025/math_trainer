...
export default function ColumnDivisionSession({
  ...
  const [preparedProblems, setPreparedProblems] = useState(() => [] as Array<ReturnType<typeof generateDivisionProblem>>);
  ...
  const problemId = `${state.problem.dividend}-${state.problem.divisor}`;
  ...
  const getHintText = () => {
    ...
    switch (currentStep.type) {
      case 'quotient_digit': {
        ...
        if (ws) return `${ws.currentNumber} ÷ ${problem.divisor} = ?`;
        return `Сколько раз ${problem.divisor} помещается?`;
      }
      case 'multiply_result': {
        const qDigit = state.quotientDigits[currentStep.position];
        if (qDigit !== null && qDigit !== undefined) return `${problem.divisor} × ${qDigit} = ?`;
        return 'Умножь делитель на частное';
      }
              ...
              <div className="flex justify-center">
                <div className="relative inline-block">
                  <div key={cardKey} className={cn(cardAnimating ? 'animate-card-exit' : 'animate-card-enter')}>
                    <ColumnDivisionDisplay state={state} currentStep={currentStep} />
                  </div>
                </div>
              </div>
