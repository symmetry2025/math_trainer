...
export default function ColumnSubtractionSession({
  ...
  const [preparedProblems, setPreparedProblems] = useState(() => [] as Array<ReturnType<typeof generateSubtractionProblem>>);
  ...
  const problemId = `${state.problem.minuend}-${state.problem.subtrahend}`;
  ...
  const getHint = () => {
    ...
    if (currentStep.type === 'borrow') {
      return `Занимаем 1 из разряда ${currentStep.position === 1 ? 'десятков' : currentStep.position === 2 ? 'сотен' : 'тысяч'}`;
    }
              ...
              <div className="flex justify-center">
                <div className="relative inline-block">
                  <div key={cardKey} className={cn(cardAnimating ? 'animate-card-exit' : 'animate-card-enter')}>
                    <ColumnSubtractionDisplay state={state} currentStep={currentStep} />
                  </div>
                </div>
              </div>
